...
SDL_Cursor *OGC_CreateSystemCursor(SDL_SystemCursor id)
{
    ...
    surface =
        SDL_CreateSurfaceFrom(cursor->width,
                              cursor->height,
...
{
    OGC_CursorData *curdata = cursor->internal;
...
void OGC_draw_cursor(SDL_VideoDevice *_this)
{
    ...
    if (mouse->cur_cursor == mouse->def_cursor) {
        ...
        SDL_MouseID mouse_id = mouse->num_sources > 0 ? mouse->sources[0].mouseID : 0;
        _OGC_Controller *controller = OGC_get_controller(mouse_id);
