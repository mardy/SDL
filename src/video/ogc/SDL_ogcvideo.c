...
static void init_display_mode(SDL_DisplayMode *mode, const GXRModeObj *vmode)
{
    ...
    mode->internal = (SDL_DisplayModeData *)vmode;
}
                              ...
                              SDL_DisplayMode *mode)
{
    ...
    GXRModeObj *vmode = (GXRModeObj *)mode->internal;
...
VideoBootStrap OGC_bootstrap = {
    "ogc", "ogc video driver",
    OGC_CreateDevice, NULL, false
};
...
void OGC_VideoQuit(SDL_VideoDevice *_this)
{
     ...
     * SDL_free() on internal. Nullify the pointers in order to avoid a
