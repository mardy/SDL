...
static void handle_accelerometer(SDL_Joystick *joystick,
                                 SDL_SensorType type,
                                 int start_axis,
                                 ...
                                 bool rotated)
{
    ...
    if (s_accelerometers_as_axes) {
        SDL_PrivateJoystickAxis(joystick, start_axis, a.x * INT16_MAX / EGC_ACCELEROMETER_RES_PER_G);
...
static int OGC_JoystickOpen(SDL_Joystick *joystick, int device_index)
{
    ...
    if (s_accelerometers_as_axes) {
        joystick->naxes += desc->num_accelerometers * 2;
    } else if (desc->num_accelerometers > 0) {
                              ...
                              Uint16 high_frequency_rumble)
{
    ...
    return egc_input_device_set_rumble(device, low_frequency_rumble, high_frequency_rumble);
}
...
static int OGC_JoystickSetSensorsEnabled(SDL_Joystick *joystick, SDL_bool enabled)
{
    ...
    return enabled ? 0 : SDL_Unsupported();
}
...
static void OGC_JoystickUpdate(SDL_Joystick *joystick)
{
    ...
    for (int i = 0; i < EGC_GAMEPAD_BUTTON_COUNT; i++) {
        ...
        if (changed & mask) {
            SDL_PrivateJoystickButton(joystick, i_button,
                                      (buttons & mask) ? SDL_PRESSED : SDL_RELEASED);
        }
        i_button++;
    }
    ...
    for (int i = 0; i < EGC_GAMEPAD_AXIS_COUNT; i++) {
        ...
        if (device->desc->available_axes & mask) {
            ...
            SDL_PrivateJoystickAxis(joystick, i_axis, value);
            i_axis++;
        }
    }
                                              ...
                                              SDL_GamepadMapping *out)
{
    ...
    *out = (SDL_GamepadMapping){
        ...
        .paddle2 = egc_map_button(buttons, EGC_GAMEPAD_BUTTON_LEFT_PADDLE1, &i_button),
        .paddle3 = egc_map_button(buttons, EGC_GAMEPAD_BUTTON_RIGHT_PADDLE2, &i_button),
        .paddle4 = egc_map_button(buttons, EGC_GAMEPAD_BUTTON_LEFT_PADDLE2, &i_button),
...
#ifdef __wii__
    if (s_accelerometers_as_axes && device->desc->num_accelerometers > 0) {
        out->leftx = (SDL_InputMapping){ EMappingKind_Axis, i_axis++ };
        out->lefty = (SDL_InputMapping){ EMappingKind_Axis, i_axis++ };
        if (device->desc->num_accelerometers > 1) {
            out->rightx = (SDL_InputMapping){ EMappingKind_Axis, i_axis++ };
            out->righty = (SDL_InputMapping){ EMappingKind_Axis, i_axis++ };
        }
    }
